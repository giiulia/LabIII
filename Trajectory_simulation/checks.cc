...
bool is_coincidence(double L, double K_x, double K_y, double x0, double y0){ //L = distance between scintillators
    double delta_x = x0 - L*K_x; //K_x = tan(theta)*cos(phi)
    double delta_y = y0 - L*K_y; //K_y = tan(theta)*sin(phi)
    if(delta_x < width/2 && delta_x > -width/2 && delta_y < depth/2 && delta_y > -depth/2){
        return true;
    }
    return false;
}
...
double f_cos2(double x){
    return 4*pow(cos(x), 2)/M_PI; //integral 
}
